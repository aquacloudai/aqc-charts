import { BaseChart, ChartError, ChartErrorCode, ChartInitError, ChartRenderError, DataValidationError, EChartsLoadError, OldBarChart, OldCalendarHeatmapChart, OldClusterChart, OldGanttChart, OldLineChart, OldPieChart, OldRegressionChart, OldSankeyChart, OldScatterChart, OldStackedBarChart, TransformError, addLogoToOption, assertValidation, createChartError, createLogoGraphic, disposeChart, getEChartsModule, getMap, isChartError, isRecoverableError, registerMap, removeLogoFromOption, safeAsync, safeSync, useChartEvents, useChartInstance, useChartOptions, useChartResize, useECharts, useLegendDoubleClick, usePrefersDarkMode, useResolvedTheme, useSystemTheme, validateChartData, validateChartProps, validateDimensions, validateFieldMapping, validateInDevelopment, validateTheme } from "./OldPieChart-D0M784YN.js";
...
function useChartComponent({ props, buildOption, chartType: _chartType }) {
	...
	const exportImage = useCallback((format = "png", opts) => {
	...
	const highlight = useCallback((dataIndex, seriesIndex = 0) => {
		const chart = getEChartsInstance();
		if (!chart) return;
	...
	const renderError = useCallback(() => /* @__PURE__ */ jsxs("div", {
...
const PieChart = forwardRef((props, ref) => {
	...
	const selectSlice = useCallback((dataIndex) => {
		const chart = getEChartsInstance();
		if (!chart) return;
		chart.dispatchAction({
			type: "pieSelect",
	...
	const unselectSlice = useCallback((dataIndex) => {
		const chart = getEChartsInstance();
		if (!chart) return;
		chart.dispatchAction({
			type: "pieUnSelect",
...
* />
*/
const StackedAreaChart = forwardRef((props, ref) => {
	const { className } = props;
...
* />
*/
const CombinedChart = forwardRef((props, ref) => {
	const { className } = props;
	const buildOption = useCallback((chartProps) => {
...
* />
*/
const ClusterChart = forwardRef((props, ref) => {
	const { className } = props;
...
* />
*/
const CalendarHeatmapChart = forwardRef((props, ref) => {
	const { className } = props;
...
* />
*/
const SankeyChart = forwardRef((props, ref) => {
	const { className } = props;
	...
	const focusNode = useCallback((nodeName) => {
		const chart = getEChartsInstance();
		if (!chart) return;
...
const GanttChart = forwardRef((props, ref) => {
	...
	const zoomToRange = useCallback((startTime, endTime) => {
		const chart = getEChartsInstance();
		if (!chart) return;
...
* />
*/
const RegressionChart = forwardRef((props, ref) => {
	const { className } = props;
...
//#endregion
export { BarChart, BaseChart, CalendarHeatmapChart, ChartError, ChartErrorBoundary, ChartErrorCode, ChartInitError, ChartRenderError, ClusterChart, CombinedChart, DataValidationError, EChartsLoadError, ExportPreviewModal, GanttChart, GeoChart, LineChart, OldBarChart, OldCalendarHeatmapChart, OldClusterChart, OldGanttChart, OldLineChart, OldPieChart, OldRegressionChart, OldSankeyChart, OldScatterChart, OldStackedBarChart, PieChart, RegressionChart, SankeyChart, ScatterChart, StackedAreaChart, TransformError, assertValidation, clusterPointsToScatterData, createChartError, darkTheme, disposeChart, extractPoints, getEChartsModule, getMap, isChartError, isRecoverableError, lightTheme, performKMeansClustering, registerMap, safeAsync, safeSync, useChartComponent, useChartErrorHandler, useChartEvents, useChartInstance, useChartOptions, useChartResize, useECharts, useFullHDExport, usePrefersDarkMode, useResolvedTheme, useSystemTheme, validateChartData, validateChartProps, validateDimensions, validateFieldMapping, validateInDevelopment, validateTheme, withChartErrorBoundary };