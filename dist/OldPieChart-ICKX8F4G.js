...
import React, { forwardRef, useCallback, useEffect, useImperativeHandle, useMemo, useRef, useState, useSyncExternalStore } from "react";
import { jsx, jsxs } from "react/jsx-runtime";

//#region src/utils/echarts.ts
const ecStatTyped = ecStat;
if (ecStatTyped.transform) {
	if (ecStatTyped.transform.clustering) echarts.registerTransform(ecStatTyped.transform.clustering);
	if (ecStatTyped.transform.regression) echarts.registerTransform(ecStatTyped.transform.regression);
	if (ecStatTyped.transform.histogram) echarts.registerTransform(ecStatTyped.transform.histogram);
}
/**
...
function useChartInstance({ containerRef, onChartReady }) {
	...
	const disposeChart$1 = useCallback(() => {
...
function computeOptionHash(option) {
	...
	if (Array.isArray(obj)) {
		...
		const first = JSON.stringify(obj[0]);
		const last = obj.length > 1 ? JSON.stringify(obj[obj.length - 1]) : "";
		const mid = obj.length > 2 ? JSON.stringify(obj[Math.floor(obj.length / 2)]) : "";
...
function useChartOptions({ chartInstance, option, theme, notMerge = true, lazyUpdate = true }) {
	...
	const lastThemeRef = useRef(void 0);
	useEffect(() => {
...
function useLegendDoubleClick({ chartInstance, onLegendDoubleClick, onSeriesDoubleClick, delay = 300, enableAutoSelection = false }) {
	...
	const lastClickType = useRef(null);
	const selectedLegends = useRef(new Set());
	const allLegendsVisible = useRef(true);
	const prevChartInstanceRef = useRef(null);
	useEffect(() => {
	...
	const handleItemClick = useCallback((params, event, type = "legend") => {
		if (!chartInstance) return;
	...
	const handleLegendClick = useCallback((params, event) => {
		handleItemClick(params, event, "legend");
	}, [handleItemClick]);
	const handleSeriesClick = useCallback((params, event) => {
