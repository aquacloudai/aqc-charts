...
interface WaterTypologyTestProps {
  theme: 'light' | 'dark';
  colorPalette: string[];
  onInteraction: (data: string) => void;
}

// Marine environment data for different water types
const marineData = {
  coastal: [
  ...
  fjord: [
    { name: 'norway-coastline', value: 92.1 },
    { name: 'water-types', value: 87.4 },
  ],
  ocean: [
    { name: 'norway-coastline', value: 76.3 },
    { name: 'water-types', value: 82.7 },
  ],
};
...
export function WaterTypologyTest({ theme, colorPalette, onInteraction }: WaterTypologyTestProps) {
  ...
  const handleMapLoad = useCallback(() => {
    onInteraction('Water typology map loaded successfully!');
  }, [onInteraction]);

  const handleMapError = useCallback((error: Error) => {
    onInteraction(`Water typology map loading failed: ${error.message}`);
  }, [onInteraction]);
    ...
    <div>
      ...
      <ExampleCard title="🌊 Norwegian Water Typologies" theme={theme}>
        <p style={{
          ...
          This uses the water typologies SVG map with marine environment data.
        </p>
